...
public class TransactionService {
    ...
    public Page<TransactionResponse> listByUserBetween(UUID userId,
        ...
        return transactionRepository
                .findAllByUser_IdAndTransactionDateBetweenOrderByTransactionDateDesc(
                        userId, desde.atStartOfDay(), hasta.atTime(LocalTime.MAX), pageable)
                .map(mapper::toResponse);
    }
    ...
    public ExpenseSummaryResponse expenseSummary(UUID userId, LocalDate desde, LocalDate hasta) {
        ...
        long fills = transactionRepository
                ...
                .getTotalElements();
        var byMonth = transactionRepository.sumByMonth(userId, start, endTime)
                .stream()
                .map(mapper::toMonthlyStat)
                .toList();
        return new ExpenseSummaryResponse(desde, hasta, total, savings, liters, fills, byMonth);
    }
    ...
    public TransactionResponse register(TransactionCreateRequest req) {
        ...
        FuelType fuelType = fuelTypeRepository.findById(req.fuelTypeId())
                .orElseThrow(() -> new ResourceNotFoundException("Fuel type not found: " + req.fuelTypeId()));

        CardProduct cardProduct = null;
        if (req.cardProductId() != null) {
            cardProduct = cardProductRepository.findById(req.cardProductId())
                    .orElseThrow(() -> new ResourceNotFoundException("Card product not found: " + req.cardProductId()));
        }
        Discount discount = null;
        if (req.discountId() != null) {
            discount = discountRepository.findById(req.discountId())
                    .orElseThrow(() -> new ResourceNotFoundException("Discount not found: " + req.discountId()));
        }
        ...
        Transaction entity = Transaction.builder()
                .user(user).vehicle(vehicle).station(station).fuelType(fuelType)
                .cardProduct(cardProduct).discount(discount)
                .unitPrice(req.unitPrice()).liters(req.liters())
                .grossAmount(grossAmount).discountAmount(discountAmount)
                .finalAmount(grossAmount.subtract(discountAmount))
                .transactionDate(req.transactionDate() != null ? req.transactionDate() : LocalDateTime.now())
                .notes(req.notes())
                .build();
