package cl.bencinaenlinea.bencinera.api;

import cl.bencinaenlinea.bencinera.application.dto.*;
import cl.bencinaenlinea.bencinera.application.service.BencineraService;
...
public class BencineraController {

    private final BencineraService bencineraService;

    @GetMapping
    @Operation(summary = "Listar bencineras paginado")
    public Page<BencineraSummaryResponse> listar(@ParameterObject Pageable pageable) {
        return bencineraService.listar(pageable);
    }

    @GetMapping("/cercanas")
    @Operation(summary = "Buscar bencineras dentro de un radio (km), ordenadas por distancia")
    public List<BencineraSummaryResponse> cercanas(
    ...
    public List<BencineraSummaryResponse> porComuna(@PathVariable @Positive Integer comunaId) {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Bencinera encontrada"),
            @ApiResponse(responseCode = "404", description = "No existe", content = @Content)
    })
    public BencineraResponse buscar(@PathVariable UUID id) {
        return bencineraService.buscarPorId(id);
    }

    @PostMapping
    @Operation(summary = "Registrar una nueva bencinera (uso administrativo o sync)")
    public ResponseEntity<BencineraResponse> crear(@Valid @RequestBody BencineraCreateRequest req,
                                                   UriComponentsBuilder uriBuilder) {
        BencineraResponse creada = bencineraService.crear(req);
    ...
    public BencineraResponse actualizar(@PathVariable UUID id,
                                        @Valid @RequestBody BencineraUpdateRequest req) {
